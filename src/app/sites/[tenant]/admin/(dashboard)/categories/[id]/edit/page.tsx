...
export default function EditCategoryPage() {
      ...
      try {
        ...
        if (business) {
          ...
          if (cat) {
            ...
            setValue('has_page', cat.has_page);
          }
        }
      } catch (error) {
  ...
  const onSubmit = async (data: CategoryFormValues) => {
    ...
    try {
      ...
      await updateCategory(id as string, categoryData);
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-zinc-400 gap-4">
        ...
        <p className="text-[10px] font-bold uppercase tracking-[0.2em]">Cargando Categoría...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 pb-20">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight uppercase">Editar Categoría</h1>
          <p className="text-zinc-500 mt-1 font-medium tracking-wide text-sm">Modifica la organización de tu catálogo</p>
        </div>
      </div>
